...
| Команда | Действие |
|---|---|
| `/add_chat @name` или `t.me/+hash` | поставить чат в очередь на вступление |
| `/remove_chat @name` | юзербот выйдет из чата |
| `/list_chats` | статусы и дата последнего сообщения |
| `/add_intent dog_boarding Передержка собак` | интент + топик в форуме |
| `/set_prompt dog_boarding <критерии>` | критерии для LLM-классификатора |
| `/set_prefilter dog_boarding передержк, выгул, /собак[аеу]/` | дешёвый фильтр |
...
Интент начинает работать, когда у него есть **и** промпт, **и** префильтр
...
- Лимиты юзербота консервативные (главный риск проекта — бан аккаунта):
  `JOIN_DAILY_LIMIT` в сутки, случайные паузы 5–15 мин между вступлениями,
  сверка раз в 2 часа с паузами между чатами, никакой рассылки.
