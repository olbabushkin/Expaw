...
| `/tune_prompt dog_boarding` | улучшить критерии и префильтр по реакциям 👍/👎 |
...
**Обратная связь:** любой участник группы ставит 👍 (верно) или 👎 (ложное
срабатывание) на пост в топике — это копится в таблице `feedback` как
обучающая выборка. `/tune_prompt <code>` отдаёт LLM текущие критерии вместе
с размеченными примерами, сохраняет улучшенную версию и обновляет префильтр.
**Каждые 5 новых реакций по интенту тюнинг запускается автоматически**,
отчёт приходит в топик «Система». Префильтр при тюнинге только расширяется
(union со старым) — покрытие не может сузиться, ничего не теряется; сверх
того проверяется, что все лайкнутые сообщения проходят префильтр, иначе —
предупреждение. Фидбек переживает `/recalc`, выборка накапливается между
итерациями.

**Один интент на сообщение:** классификатор выбирает максимум один — самый
подходящий — интент (страховка в коде: при нескольких `match` берётся
наибольший score).
...
userbot/    Telethon: push-слушатель, менеджер пула, сверка, login-скрипт
bot/        aiogram 3: команды админа, публикатор, тюнинг, топик «Система»
worker/     префильтр + классификация Claude, ретеншн
docs/       DEVLOG.md — история разработки, решения и грабли
```
