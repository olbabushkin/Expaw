...
При подключении чата подгружается **история за `BACKFILL_DAYS` дней**
(по умолчанию 30, максимум `BACKFILL_MAX_MESSAGES` сообщений; `0` —
только новые). Дальше сообщения приходят push'ем в реальном времени.
Перечитать историю уже подключённого чата: `/remove_chat` → `/add_chat`
(дубликаты не создаются). Интент начинает работать, когда у него есть **и** промпт, **и** префильтр
